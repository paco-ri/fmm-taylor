subroutine lap_comb_dir_eval_addsub_vec(npatches, norders, ixyzs, &
  ...
  implicit none
  integer *8 npatches,npts,ndd,ndz,ndi,nnz,ndtarg
  integer *8 norders(npatches),ixyzs(npatches+1),iptype(npatches),ipars(ndi)
  real *8 srccoefs(9,npts),srcvals(12,npts),eps,dpars(ndd)
  complex *16 zpars(ndz)
  integer *8 ntarg,nquad,nker,nptso,lwork,ndim
  integer *8 row_ptr(ntarg+1),col_ind(nnz),iquad(nnz+1),novers(npatches+1)
  complex *16 wnear(nquad),sigma(ndim,npts)
  integer *8 ixyzso(npatches+1)
...
subroutine mtxbsigma_eval_addsub(npatches,norders,ixyzs, &
  ...
  implicit none
  integer *8 npatches,npts,nnz_gc,ndtarg
  ...
  real *8 dpars
  complex *16 zpars(3)
  integer *8 ntarg,nquad_gc,nptso_gc
  integer *8 ndim
  
  integer *8 row_ptr_gc(ntarg+1),col_ind_gc(nnz_gc)
  integer *8 iquad_gc(nnz_gc+1),novers_gc(npatches)
  complex *16 wnear_gc(nquad_gc,3)
  integer *8 ixyzso_gc(npatches+1)
  ...
  integer *8 ixyzso_helm(npatches+1)
  integer *8 nnz_helm,row_ptr_helm(ntarg+1)
  integer *8 col_ind_helm(nnz_helm),nquad_helm
  integer *8 iquad_helm(nnz_helm+1)
  complex *16 rjvec(3,npts),rho(npts)
  complex *16 wnear_helm(nquad_helm)

  integer *8 novers_helm(npatches),nptso_helm
