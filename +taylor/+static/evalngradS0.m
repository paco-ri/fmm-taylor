...
%EVALGRADCURLS0 compute n . grad S0[rho]
%
%  Input arguments:
...
% Extract quadrature arrays
    ...
    gradrho = zeros(3,ntarg);

% Call layer potential evaluator
    mex_id_ = 'lpcomp_gradlap_addsub(i int[x], i int[x], i int[x], i int[x], i int[x], i double[xx], i double[xx], i int[x], i int[x], i double[xx], i double[x], i int[x], i int[x], i int[x], i int[x], i int[x], i double[x], i double[xx], i int[x], i int[x], i int[x], i double[xx], i double[x], io double[xx])';
